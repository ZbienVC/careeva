...
const ONBOARDING_STEPS = [
  ...
  },
  {
    ...
    helper: 'A floor helps filter out weak offers; a ceiling helps prioritize. Both are optional.',
    type: 'salary-range',
  },
  {
    id: 'jobType',
    ...
    <div className="grid gap-8 xl:grid-cols-[0.8fr_1.2fr]">
      <div className="premium-card p-6 md:p-7">
        ...
        <div className="mt-6 h-2 overflow-hidden rounded-full bg-white/10">
          <div className="h-full rounded-full bg-gradient-to-r from-blue-500 via-cyan-400 to-violet-500 transition-all duration-300" style={{ width: `${progress}%` }} />
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={complete ? 'step-dot-done !h-7 !w-7' : active ? 'step-dot-active !h-7 !w-7' : 'step-dot-todo !h-7 !w-7'}>
                    {complete ? <IconCheck size={13} /> : index + 1}
                  </span>
      ...
      <div className="premium-card space-y-6 p-6 md:p-8">
        ...
        {step.type === 'salary-range' && (
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="salary-min" className="field-label">Minimum (USD / year)</label>
              <input
                id="salary-min"
                type="number"
                inputMode="numeric"
                ...
                onChange={(e) => handleInputChange('desiredSalaryMin', e.target.value)}
                placeholder="e.g. 90000"
              />
            </div>
            <div>
              <label htmlFor="salary-max" className="field-label">Maximum (USD / year)</label>
              <input
                id="salary-max"
                type="number"
                inputMode="numeric"
                ...
                onChange={(e) => handleInputChange('desiredSalaryMax', e.target.value)}
